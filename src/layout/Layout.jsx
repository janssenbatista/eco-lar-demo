...
import { Leaf, LayoutDashboard, Plus, Target, Lightbulb, Calculator, BrainCircuit, User } from "lucide-react";
import {
	SidebarProvider,
	Sidebar,
	SidebarHeader,
...
function LayoutShell() {
	...
	return (
		<div className="flex min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
			{isOpen && <div className="fixed inset-0 z-20 bg-black/30 backdrop-blur-sm md:hidden" onClick={closeSidebar} />}
			<Sidebar className="border-r border-emerald-100">
									...
									return (
										<SidebarMenuItem key={item.title}>
											<Link to={item.url} className="block">
												...
												>
													<div className="flex items-center gap-3">
														<Icon className={isActive ? "h-5 w-5 text-white" : "h-5 w-5 text-emerald-600"} />
				...
				<SidebarFooter className="border-t border-emerald-100/70">
					<div className="rounded-xl border border-emerald-200/60 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 p-4">
						<p className="text-sm font-semibold text-gray-900">💚 Impacto positivo</p>
						<p className="text-xs text-gray-600">Cada hábito sustentável conta muito!</p>
					</div>
				</SidebarFooter>
			</Sidebar>
